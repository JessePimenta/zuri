...
export const Scrap = ({
  ...
  const handleMouseDown = useCallback(
    (e: React.MouseEvent) => {
      ...
      startPosRef.current = { x: left, y: top };
  ...
  const handleMouseLeave = useCallback(() => {
    if (elRef.current) elRef.current.style.zIndex = isNote ? "15" : "10";
  }, [isNote]);
  ...
  useEffect(() => {
    ...
    const handleMouseUp = () => {
      if (draggedRef.current && onDragEnd) {
        const left = parseInt(draggedRef.current.style.left || "0", 10);
        const top = parseInt(draggedRef.current.style.top || "0", 10);
        onDragEnd(left, top);
      }
